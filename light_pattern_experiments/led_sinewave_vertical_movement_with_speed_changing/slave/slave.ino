...
// Title: Sinosoidal Wave on Single strip
// This one is *slave*
...
byte intervalChangingCounter = 0;
byte intervalChangingNumOfFrames = 30;
...
{    
  ...
  if(i.update()){
    ...
    i.setInterval(100 + 102*sin((float(intervalChangingCounter)/intervalChangingNumOfFrames))*2*PI );
    intervalChangingCounter++;
    if(intervalChangingCounter >= intervalChangingNumOfFrames){
      intervalChangingCounter = 0;
    }
    i.clearExpired();
  }
  ...
  for(byte i=0; i < NUM_LED_STRIPS_SLAVE; i++){ 
    ...
    for(byte j=0; j < NUM_LEDS; j++){
      float brightness = 0.21 + 0.2*sin(( (float(ledAnimationFrameCounter)/ledAnimationNumOfFrames) + (float(j)/NUM_LEDS) )*2*PI);
